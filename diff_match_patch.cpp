...
QString Patch::toString() {
    ...
    foreach (Diff aDiff, diffs) {
        ...
        text += QString::fromUtf8(QUrl::toPercentEncoding(aDiff.text, " !~*'();/?:@&=+$,#") + '\n');
    }
...
QString diff_match_patch::diff_toDelta(const QList<Diff> &diffs) {
    ...
    foreach(Diff aDiff, diffs) {
        switch (aDiff.operation) {
            case INSERT: {
                             QString encoded = QString::fromUtf8(QUrl::toPercentEncoding(aDiff.text,
                                         " !~*'();/?:@&=+$,#"));
